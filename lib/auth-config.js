 ...
 * Clerk's `publicMetadata`, or an id in `ADMIN_USERS`, and the thing that must never drift is *which*
...
export async function getAdminUser() {
  ...
  if (!user) return null;
  if (!isAllowlistedAdmin(user.id) && user.publicMetadata?.role !== "admin") {
    return null;
  }
  return user;
}
