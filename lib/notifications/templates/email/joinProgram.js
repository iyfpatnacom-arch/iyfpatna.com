export function renderJoinProgramEmail({ name, title, schedule, location, locale }) {
  ...
  const scheduleLabel = isHi ? "समय" : "Schedule";
  const locationLabel = isHi ? "स्थान" : "Location";
  const footer = isHi
  ...
  const html = `
    <div style="font-family:sans-serif;background:#100A06;color:#FBF4EA;padding:32px;">
      ...
      <table style="margin:16px 0;">
        <tr><td style="color:#F2A63B;padding-right:12px;">${scheduleLabel}</td><td>${schedule}</td></tr>
        ${location ? `<tr><td style="color:#F2A63B;padding-right:12px;">${locationLabel}</td><td>${location}</td></tr>` : ""}
      </table>
