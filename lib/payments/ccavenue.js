...
/**
 * CCAvenue non-seamless (billing page) integration — the same flow the
 * Vrindavan Yatra portal runs.
 *
 ...
 * where CCAvenue cannot reach us anyway.
 */
export function getCcavenueConfig() {
  ...
  const mode = process.env.CCAVENUE_ENCRYPTION === "aes256" ? "aes256" : "aes128";
  ...
  const environment = process.env.CCAVENUE_ENV === "production" ? "production" : "test";

  const endpoint =
    (process.env.CCAVENUE_ENDPOINT || "").trim() ||
    HOSTS[environment] + TRANSACTION_PATH[mode];

  return { merchantId, accessCode, workingKey, mode, environment, endpoint, siteUrl };
}
 ...
 *
 *   "ccavenue"  the live (or test) gateway is configured.
...
export function paymentMode() {
  if (isPaymentConfigured()) return "ccavenue";
...
export function encrypt(plainText, workingKey, mode = "aes128") {
  if (mode === "aes256") {
    ...
    return iv.toString("hex") + Buffer.concat([body, cipher.getAuthTag()]).toString("hex");
  }

  const cipher = crypto.createCipheriv("aes-128-cbc", aes128Key(workingKey), AES128_IV);
...
export function decrypt(encryptedHex, workingKey, mode = "aes128") {
  ...
  if (mode === "aes256") {
    ...
    const decipher = crypto.createDecipheriv("aes-256-gcm", workingKey, buffer.subarray(0, 12));
  ...
  const decipher = crypto.createDecipheriv("aes-128-cbc", aes128Key(workingKey), AES128_IV);
...
/**
 * Builds the auto-submitting form's payload for one enrolment.
 *
 * The amount comes from the stored document, never from the browser: the
 * enrolment row is the only authority on what this seat costs.
 */
export function buildPaymentRequest(enrollment, { lang = "hi", config } = {}) {
  ...
  const fields = {
    ...
    tid: String(Date.now()),
    billing_name: cleanName(enrollment.name, 60),
    billing_tel: clean(enrollment.phone, DIGITS, 20),
    billing_email: clean(enrollment.email, EMAIL_CHARS, 70),
    billing_country: "India",
    // Echoed back in the response: what kind of order this is, which course,
    // and which language to return the visitor to.
    merchant_param1: "course",
    merchant_param2: clean(enrollment.courseSlug, PARAM_CHARS, 100),
 ...
 */
export function toPaymentUpdate(response, enrollment) {
  ...
  const amountMismatch =
    ...
    (!Number.isFinite(returnedAmount) ||
      ...
      (response.currency || "INR") !== (enrollment.currency || "INR"));

  return {
    status,
    provider: response.provider || "ccavenue",
    ...
    failureMessage:
      status === "success" ? null : response.failure_message || response.status_message || null,
