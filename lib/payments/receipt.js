...
/** The gateway names the rail; the receipt names it the way a person would. */
const MODES = {
...
async function drawPassPage(pdf, enrollment, details, { regular, bold, serif }) {
  ...
  const centre = (value, y, size, font = regular, color = INK) => {
    ...
    page.drawText(safe, { x: (PAGE_W - font.widthOfTextAtSize(safe, size)) / 2, y, size, font, color });
  };
...
export async function buildReceiptPdf(enrollment, details) {
  ...
  if (enrollment.mrp && enrollment.mrp > enrollment.amount) {
    right(
      `Course donation ${money(enrollment.mrp)}  |  Discount ${money(enrollment.mrp - enrollment.amount)}`,
      y - 64,
      8.5,
      regular,
      MUTED
