...
export const TOOLS = [
  ...
  {
    /**
     * The one entry whose `href` leaves `/playground`.
     *
     * The campus map is a top-level section with its own nav item, because a
     * first-time visitor looking for the restaurant is not going to think to
     * open something called Playground. But someone already in here, browsing
     * the tools, is exactly the person who would use it — so it appears in the
     * grid too, and the link just goes where it lives.
     */
    key: "campus",
    href: "/campus",
    icon: "MapPin",
