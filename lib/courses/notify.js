...
import { absoluteUrl, orderStatusPath, passImagePath, receiptPath } from "@/lib/payments/order-link";
...
export async function sendEnrollmentConfirmation(enrollment) {
  try {
    ...
    const [statusPath, receipt, whatsappUrl, passImage] = await Promise.all([
      orderStatusPath(enrollment.orderId, locale),
      receiptPath(enrollment.orderId),
      whatsappGroupFor(course),
      passImagePath(enrollment.orderId),
    ]);
    ...
    const { subject, html } = renderCourseEnrollmentEmail({
      ...
      whatsappUrl,
      passImageUrl: absoluteUrl(passImage),
    });
