...
export const ORG = {
  ...
  email: process.env.NEXT_PUBLIC_CONTACT_EMAIL || "official.iyfpatna@gmail.com",
  phone: process.env.NEXT_PUBLIC_CONTACT_PHONE || "+91 90310 54013",
  ...
  policyUpdated: process.env.NEXT_PUBLIC_POLICY_UPDATED || "2026-09-22",
};
...
/**
 * Departure day of the Vrindavan yatra, as an IST calendar date. The promo
 * modal counts down to it; the printed date itself lives in the messages.
 */
export const YATRA_DEPARTURE = "2026-10-24";
...
/**
 * Whether the yatra is announced on this site at all.
 *
 * On by default now that yatra bookings take live payments. Set
 * NEXT_PUBLIC_YATRA_ENABLED=false (or 0/no/off) to pull the nav item, the
 * home-page callout, the ticker, the darshan pill and the promo modal together
 * — nothing else needs editing.
 *
 * Read as a whole literal rather than `process.env[name]` because Next inlines
 * NEXT_PUBLIC_* at build time only when the reference is written out in full;
 * flipping it therefore takes a rebuild, not just a restart.
 */
export const YATRA_ENABLED = !["0", "false", "no", "off"].includes(
  String(process.env.NEXT_PUBLIC_YATRA_ENABLED || "").trim().toLowerCase(),
);
...
export const WHATSAPP_GROUP_URL =
  ...
  "https://chat.whatsapp.com/DSKWMXaVV2t0Ccm5mCwZZO?s=sh&p=a&mlu=4&ilr=4";

/**
 * The temple's general WhatsApp group — for everyone who comes to the temple,
 * not only the youth forum. Offered on the schedule page, and reached from
 * the printed QR code at /schedule#temple-whatsapp.
 *
 * Editable at /admin/settings like the youth group; this is the fallback when
 * nothing is saved there.
 */
export const TEMPLE_WHATSAPP_GROUP_URL =
  process.env.NEXT_PUBLIC_TEMPLE_WHATSAPP_GROUP_URL ||
  "https://chat.whatsapp.com/IR6FItitYPyJhN2oiYks7X?mlu=0&s=qt&p=a";
 ...
 *
 * The yatra entry is present only while `YATRA_ENABLED` is set, so the header,
 * the mobile drawer and the footer all drop it together — they each map over
 * this one list rather than deciding for themselves.
 */
 ...
 *
 * The payment gateway's review checks for each of privacy, terms, refund,
 * shipping and contact by name, so none of them can be dropped while the site
 * takes course fees.
 */
...
export const PARENT_BAND = {
  ...
  temple: "Sri Sri Radha Banke Bihari Temple , Patna",
...
export const HERO_IMAGE = 'https://ik.imagekit.io/mnkh9j9dw/IYF/WhatsApp%20Image%202026-09-21%20at%2008.50.53.jpeg';
...
export const FALLBACK_COURSES = [
  ...
  },
  {
    ...
    description: {
      hi: "फोकस, स्पष्टता और आंतरिक स्थिरता पर 2 दिन की मास्टरक्लास — व्यावहारिक ध्यान और प्राचीन ज्ञान।",
      en: "A 2-day masterclass on focus, clarity and inner stability — practical meditation and ancient wisdom.",
    },
    duration: { hi: "2 दिन · 6 सत्र", en: "2 days · 6 sessions" },
    level: "beginner",
  },
  {
